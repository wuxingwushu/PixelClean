...
/**
 * Direction bitmasks for neighbor rules.
 * Each bit corresponds to one direction: a neighbor rule only applies
 * in directions whose bits are set.
 */
constexpr unsigned DIR_UP    = 0x1; // bit 0 → direction 0
constexpr unsigned DIR_LEFT  = 0x2; // bit 1 → direction 1
constexpr unsigned DIR_RIGHT = 0x4; // bit 2 → direction 2
constexpr unsigned DIR_DOWN  = 0x8; // bit 3 → direction 3
constexpr unsigned DIR_ALL   = 0xF; // all 4 directions
...
private:
  ...
  static std::vector<std::array<std::vector<unsigned>, 4>> generate_propagator(
    ...
    for (auto neighbor : neighbors) {
      ...
      unsigned dirMask = std::get<4>(neighbor);
      ...
      auto add = [&](unsigned action, unsigned direction) {
        if (!(dirMask & (1u << direction))) return;
