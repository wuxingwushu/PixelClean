...
namespace PhysicsBlock
{
    ...
    PhysicsParticle *PhysicsLiquid::AddParticle(Vec2_ pos, FLOAT_ mass, FLOAT_ friction)
    {
        ...
        mWorld->AddObject(p); // 注册到世界：网格搜索/重力/地形与物体碰撞全部复用
    ...
    void PhysicsLiquid::ResolveSolidOverlap()
    {
        const size_t n = mParticles.size();
        if (n == 0)
        {
        ...
        // 亚像素级投影：步长 0.02，上限 0.55h。
        // 旧实现从 0.25 起步、最大 1.0：粒子仅穿透 0.005 时也会被整体抛出 0.25，
        // 速度回代 v=Δx/dt 立刻产生 25u/s 的向上速度 → 每帧"发射"一次 →
        // 池底/凸起/墙角处永远在碰撞飞溅。
        // 0.02 步长把逐帧修正量压到真实穿透深度量级（0.005~0.05），
        // 回代速度只有 1~5 u/s 的微小震荡，且与压力/粘滞循环收敛成平静水面。
        const FLOAT_ stepBase = 0.02f;
        const FLOAT_ maxStep = param.h * FLOAT_(0.55); // 略大于单帧最大 PBF 挤入量（0.5h），角落不积累
        ...
        auto NearestFree = [&](Vec2_ from, auto&& IsFree) -> Vec2_
        {
            Vec2_ best = from;
            FLOAT_ bestLen = FLOAT_MAX;
            for (int di = 0; di < 8; ++di)
            {
                for (FLOAT_ len = stepBase; len <= maxStep + FLOAT_(1e-4); len += stepBase)
                {
        ...
        for (size_t i = 0; i < n; ++i)
        {
            ...
            {
                pos = NearestFree(pos, [&](const Vec2_ &cand) { return !map->FMGetCollide(cand); });
            }
            ...
            // ── 网格形状固体：DropCollision 判定 + 方向采样找最近空闲点 ──
            for (auto *s : shapes)
            {
                if (s == nullptr)
                {
                ...
                if (Modulus(d) > s->radius + maxStep)
                {
                    continue; // 远距粗筛
                }
                if (!s->DropCollision(pos).Collision)
                {
                    continue;
                }
                pos = NearestFree(pos, [&](const Vec2_ &cand) { return !s->DropCollision(cand).Collision; });
            }
    ...
    void PhysicsLiquid::ApplySolidCoupling(FLOAT_ time)
    {
        ...
        const FLOAT_ queryPad = param.h * FLOAT_(0.5);

        auto ApplyTo = [&](PhysicsParticle *solid)
        {
            ...
            const FLOAT_ Rq = R + queryPad;
            const FLOAT_ Rq2 = Rq * Rq;
            const Vec2_ c = solid->PFGetPos();

            // 局部浸没估算：统计固体周围液体粒子数
            mWorld->mGridSearch.Get(c, Rq, mSearchV);
            ...
            for (auto *o : mSearchV)
            {
                ...
                const Vec2_ d = o->PFGetPos() - c;
                if (ModulusLength(d) <= Rq2)
                {
                    ++count;
                }
            }
            ...
            // 浸没比例 = 周围粒子数 / 完全浸没时应有的粒子数（0..1）
            const FLOAT_ nFull = (FLOAT_)M_PI * Rq2 / areaPerParticle;
            const FLOAT_ frac = std::clamp((FLOAT_)count / nFull, FLOAT_(0), FLOAT_(1));
            if (frac <= FLOAT_(0.01))
            {
                return;
            }
