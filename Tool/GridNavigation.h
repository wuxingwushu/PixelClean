...
// 獲取邊框是否有障礙物
template <typename TData>
bool GridNavigation<TData>::GetFK(int x, int y, int K) {
    int XY = x + K;
    int K1 = y + K, K2 = y - K;
...
// 獲取網格點的碰撞距離
template <typename TData>
unsigned int GridNavigation<TData>::GetRawg(int x, int y, unsigned int stin) {
...
// 設置點周圍一圈網格
template <typename TData>
void GridNavigation<TData>::SetRawg(int x, int y, int K) {
    int XY = x + K;
    int K1 = y + K, K2 = y - K;
    for(int j = x - K; j <= XY; ++j) {
...
// 添加點
template <typename TData>
void GridNavigation<TData>::add(int x, int y) {
    at(x, y) = 0;
...
// 設置點周圍一圈網格
template <typename TData>
void GridNavigation<TData>::SetPopRawg(int x, int y, int K) {
    int XY = x + K;
    int K1 = y + K, K2 = y - K;
    for(int j = x - K; j <= XY; ++j) {
...
// 移除點
template <typename TData>
void GridNavigation<TData>::pop(int x, int y) {
    at(x, y) = GetRawg(x, y);
