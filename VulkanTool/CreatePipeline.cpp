...
namespace VulKan
{
	...
	Pipeline *SpotPipeline(Pipeline *Pipeline, Device *Device)
	{
		...
		attributeDes[1].binding = 0; // 大小（新增）
		attributeDes[1].location = 1;
		attributeDes[1].format = VK_FORMAT_R32_SFLOAT;
		attributeDes[1].offset = sizeof(float) * 3; // 紧接位置之后

		attributeDes[2].binding = 0;
		attributeDes[2].location = 2; // 颜色
		attributeDes[2].format = VK_FORMAT_R32G32B32A32_SFLOAT;
		attributeDes[2].offset = sizeof(float) * 4; // 位置(3) + 大小(1)
	...
	Pipeline *CirclePipeline(Pipeline *Pipeline, Device *Device)
	{
		...
		attributeDes[1].binding = 0;
		attributeDes[1].location = 1; // 半径
		attributeDes[1].format = VK_FORMAT_R32_SFLOAT;
		attributeDes[1].offset = sizeof(float) * 3; // 位置大小

		attributeDes[2].binding = 0;
		attributeDes[2].location = 2; // 颜色
		attributeDes[2].format = VK_FORMAT_R32G32B32A32_SFLOAT;
		attributeDes[2].offset = sizeof(float) * 4; // 位置 + 半径
	...
	Pipeline *DamagePromptPipeline(Pipeline *Pipeline, Device *Device)
	{
		...
		VulKan::Shader *shaderVertex = new VulKan::Shader(Device, DamagePromptV_spv, VK_SHADER_STAGE_VERTEX_BIT, "main");
		shaderGroup.push_back(shaderVertex);

		VulKan::Shader *shaderFragment = new VulKan::Shader(Device, DamagePromptF_spv, VK_SHADER_STAGE_FRAGMENT_BIT, "main");
		shaderGroup.push_back(shaderFragment);
	...
	Pipeline *UVDynamicDiagramPipeline(Pipeline *Pipeline, Device *Device)
	{
		...
		attributeDes[0].binding = 0;
		attributeDes[0].location = 0;
		attributeDes[0].format = VK_FORMAT_R32G32_SFLOAT;
		attributeDes[0].offset = 0;
		attributeDes[1].binding = 1;
		attributeDes[1].location = 1;
		attributeDes[1].format = VK_FORMAT_R32_SINT;
		attributeDes[1].offset = 0;
		...
		layoutBindings[2].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER; // 绑定类型
		layoutBindings[2].binding = 2;										  // 那个Binding通道
		layoutBindings[2].stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
		layoutBindings[2].descriptorCount = 1; // 多少个数据

		layoutBindings[3].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER; // 绑定类型
		layoutBindings[3].binding = 3;										  // 那个Binding通道
		layoutBindings[3].stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
		layoutBindings[3].descriptorCount = 1; // 多少个数据
	...
	{
		LOGD("[CreatePipeline] BlockWorldPipeline");
		// 设置视口
		VkViewport viewport = {};
		viewport.x = 0.0f;
		viewport.y = (float)Global::mHeight;
		...
		VulKan::Shader *shaderVertex = new VulKan::Shader(Device, BlockWorldV_spv, VK_SHADER_STAGE_VERTEX_BIT, "main");
		shaderGroup.push_back(shaderVertex);

		VulKan::Shader *shaderFragment = new VulKan::Shader(Device, BlockWorldF_spv, VK_SHADER_STAGE_FRAGMENT_BIT, "main");
		shaderGroup.push_back(shaderFragment);
		...
		Pipeline->mRasterState.frontFace = VK_FRONT_FACE_CLOCKWISE;
		...
		if (vkCreateDescriptorSetLayout(Device->getDevice(), &createInfo, nullptr, &Pipeline->DescriptorSetLayout) != VK_SUCCESS)
		{
			LOGE("[CreatePipeline] BlockWorldPipeline: failed to create descriptor set layout");
			throw std::runtime_error("Error: failed to create descriptor set layout");
		}
